(function (root, factory) {
    ...
    function keywordTransform(map) {
        // Use a JavaScript Map to map keywords to their corresponding token
        // type unless Map is unsupported, then fall back to using an Object:
    ...
    Lexer.prototype.formatError = function (token, message) {
        ...
        const reg = new RegExp(
            `^([^\\n]*\\n){${firstDisplayedLine - 1}}(([^\\n]*\\n){${
                lastDisplayedLine - firstDisplayedLine
            }})`
        );
        const displayedLines = (
            reg.exec(this.buffer)?.[2]?.split('\n') ?? []
        ).slice(0, 5);
        ...
        const errorLines = [
            ...
            ...displayedLines
                .map((line, i) => {
                    ...
                    const lineOutput = `${
                        ' '.repeat(lastLineDigits - String(lineNo).length) +
                        lineNo
                    }  ${line}`;
                    return lineNo === token.line
                        ? [
                              lineOutput,
                              ' '.repeat(lastLineDigits + token.col + 1) + '^',
                          ]
                        : lineOutput;
                })
                .flat(),
